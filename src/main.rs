...
use influxdb::{Client, InfluxDbWriteable, ReadQuery, Error};
use rand::Rng;
...
pub async fn fetch_main(cmc_api_key: &str) -> Result<(), Error> {
    loop {
        ...
        println!("[{}] Waking up to get data and persist to InfluxDB", timestamp);
        let listing = CryptoListing::fetch_web(&cmc_api_key);
        listing.await.persist_db().await.expect("TODO: panic message");
...
impl CryptoListing {
    pub async fn fetch(file: &str) -> CryptoListing {
       let mut fi = File::open(file).unwrap();
        ...
        let cryptolisting: CryptoListing = serde_json::from_str(&data).expect("JSON was not well-formatted");
        cryptolisting
    }

    pub async fn fetch_web(api_key: &str) -> CryptoListing {
        let client = reqwest::Client::new();
        let cmc_url = "https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest?start=1&limit=500&convert=USD";
        let res =
            client
                .get(cmc_url)
        ...
        let cryptolisting: CryptoListing = serde_json::from_str(&res).expect("JSON was not well-formatted");
        cryptolisting
    ...
    pub async fn persist_db(&self) -> Result<(), Error> {
        ...
        for data in self.data.iter() {
           let cmc_price = CmcPrice {
            ...
            cmc_price.insert_to_db(&data.symbol.to_string().to_lowercase()).await.expect("TODO: panic message");
